...
def get_posts():
    ...
    date_params={"pub_date_start":two_weeks.strftime("%Y-%m-%d"),"pub_date_end":week_ago.strftime("%Y-%m-%d"),"period_start":two_weeks.strftime("%Y-%m-%d"),"period_end":now.strftime("%Y-%m-%d")}
    combined={}
    for sort_key in ["views","avg_engaged","returning_visitors","mobile_views","desktop_views"]:
        for post in parsely_get("/analytics/posts",{**date_params,"sort":sort_key}):
            url=post.get("url","")
            if url and url not in combined: combined[url]=post
            elif url:
...
def score_articles(posts):
    ...
    for post in posts:
        try:
            ...
            returning=z_to_pct(ret_pct,b["ret_pct_mean"],b["ret_pct_std"])
            composite=round(reach*0.30+depth*0.50+returning*0.20,1)
            ...
            scored.append({"url":post.get("url",""),"title":meta.get("title","(no title)"),
                ...
                "views":views,"reach":reach,"depth":depth,"returning":returning,"composite":composite,
  ...
  document.getElementById('rightPanel').innerHTML=
    ...
    '<div class="card"><h2>How It Was Accessed</h2>'+
      '<div class="dev-bar-full">'+
        '<div class="dev-seg" style="width:'+mob+'%;background:#005195" title="Mobile '+mob+'%"></div>'+
        '<div class="dev-seg" style="width:'+desk+'%;background:#3498db" title="Desktop '+desk+'%"></div>'+
        '<div class="dev-seg" style="width:'+tab+'%;background:#c8d0d8" title="Tablet '+tab+'%"></div>'+
      '</div>'+
...
function render() {
  ...
  filtered=ROWS.filter(r=>
    (wk==='all'||r.week_scored===wk)&&(sec==='all'||r.section_norm===sec)&&
...
['search','weekSel','secSel','sortSel'].forEach(id=>document.getElementById(id).addEventListener('input',render));
...
.leg-returning{{background:rgba(26,188,156,.15);color:#1abc9c}}
...
.sb-label{{width:90px;font-size:10px}}
  ...
  <div class="table-panel">
    <div class="table-header">Story Rankings
      <div class="pill-legend">
        ...
        <span class="leg-returning">Retention</span>
      </div>
    </div>
    <table>
      <thead><tr>
...
def main():
    ...
    for s in new_scored[:5]: print(f"    [{s['composite']}] {s['title'][:60]}")
